...
module.exports = class Crawler {
    ...
    static async searchCourses(searchFromLocalFile) {
        if (searchFromLocalFile && await fs.exists(path.resolve(__dirname, '../json/search-courses.json'))) {
            ...
            const courses = require(path.resolve(__dirname, '../json/search-courses.json'))
        ...
        await fs.writeFile(path.resolve(__dirname, '../json/search-courses.json'), JSON.stringify(lessons, null, 2), 'utf8')
        return lessons;
    }
    ...
    async getCoursesForDownload(page, link, { all }) {
        ...
        let specialCourses = [];
        if (await fs.exists(path.resolve(__dirname, '../json/search-courses.json'))) {//!all &&
            console.log('LOAD COURSE FROM LOCAL FILE');
            specialCourses = require(path.resolve(__dirname, '../json/search-courses.json'))
